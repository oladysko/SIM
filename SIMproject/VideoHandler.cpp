...
DICOMBasedFrame::DICOMBasedFrame(int *frame, int width, int height, DICOMBasedFrame *next)
{
	this->frame = frame;
	this->width = width;
	this->height = height;
...
void VideoHandler::addNewFrame(int *frame)
	{
		if (last != NULL)
		{
			last->next = new DICOMBasedFrame(frame, width, height, NULL);
			last = last->next;
			frameN++;
		}else{
			last = new DICOMBasedFrame(frame, width, height, NULL);
			head=last;
			current = head;
			frameN++;
		}
...
void VideoHandler::addNewFrame(int **frame)
	{
		...
		if (last != NULL)
		{
			last->next = new DICOMBasedFrame(oneDFrame, width, height, NULL);
			last = last->next;
			frameN++;
		}
		else{
			last = new DICOMBasedFrame(oneDFrame, width, height, NULL);
			head = last;
			current = head;
			frameN++;
		}
